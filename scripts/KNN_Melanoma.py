# -*- coding: utf-8 -*-
"""
Created on Mon Jul 29 16:57:28 2024
...
class Classifier:
   ...
   def train(self, X_train, y_train, n_neighbors):
         ...
         return knn, scaler, class_weights
...
if __name__ == "__main__":
    ...
    # train_images = knn_classifier.load_images(train_dir,train_dir_hsv)
    # test_images = knn_classifier.load_images(test_dir, test_dir_hsv)
    
    train_csv_path = "C:\\Users\\estel\\STAGE\\Dataset\\features_train_knn.csv"
    test_csv_path = "C:\\Users\\estel\\STAGE\\Dataset\\features_test_knn.csv"
    ...
    train_features = knn_classifier.extract_features(train_dir_hsv, label='train')
    print("Extracting features for test images...")
    test_features = knn_classifier.extract_features(test_dir_hsv, label='test')
    ...
    X_train, y_train = knn_classifier.prepare_data(train_features, label_mapping)
    X_test, y_test = knn_classifier.prepare_data(test_features, label_mapping)
    
    # Entrainement du modèle
    knn_model, scaler, class_weights = knn_classifier.train(X_train, y_train, n_neighbors)

    # Prédictions sur le jeu de test
    image_paths_test = [feature['image_path'] for feature in test_features]
    y_pred, y_pred_proba, predictions = knn_classifier.predict(knn_model, scaler, X_test, image_paths_test)
    
    
    # Enregistrement des résultats 
    knn_classifier.save_features_to_csv(train_features, test_features, y_test, y_pred, y_pred_proba, train_csv_path, test_csv_path)
    
    # Évaluation du modèle
    knn_classifier.evaluate(y_test, y_pred, y_pred_proba, class_weights=class_weights)
    
    test_features_df = pd.read_csv(test_csv_path)
